...
def seg_inference(iterator, model, image_folder, inp_slide=""):
    ...
    for x_batch, y_batch, info in iterator:
        ...
        if current_image != filekey:
            ...
            wsm_writer.write(
                path=f"tempoutput/segoutput/{inp_slide}",
                spacing=spacing,
                dimensions=shape,
...
class TIGERSegDet(object):
    ...
    def stroma_to_mask(
        self,
        segpath="tempoutput/segoutput/*.tif",
        bulkpath="tempoutput/segoutput/*.tif",
    ):
    ...
    def process(self):
        ...
        # K.clear_session()
        # del model
        wsm = WholeSlideImage(tissue_file, backend="asap")
        wsm_slide80 = wsm.get_slide(8.0)
        count = np.count_nonzero(wsm_slide80)

        # Leaderboard 1
        if count < 50000:
            print('ROI mode.... predicting lymphocytes')
            iterator = create_batch_iterator(
                mode="training",
                user_config=f"./{self.seg_config}/slidingwindowconfig.yml",
                presets=("slidingwindow", "folders"),
        ...
        try:
            shutil.copyfile(
                f"tempoutput/segoutput/{slide_file}",
                f"{self.output_folder}/images/breast-cancer-segmentation-for-tils/{slide_file}",
            )
        except OSError:
        ...
        try:
            shutil.copyfile(
                f"tempoutput/bulkoutput/{slide_file_xml}",
                f"{self.output_folder}/bulks/{slide_file_xml}",
            )
        except OSError:
            print(f"Failed copying: Could not copy bulk {slide_file_xml}")
            self.stroma_to_mask()
            try:
                self.detection_in_mask(slide_file)
            except ValueError:
                print('Failed detection after copybulk failed')
                det_result = dict(
                    type="Multiple points", points=[], version={"major": 1, "minor": 0}
                )
                write_json(det_result, f"{self.output_folder}/detected-lymphocytes.json")
        ...
        self.stroma_to_mask(
            segpath="tempoutput/segoutput/*.tif", bulkpath="tempoutput/bulkoutput/*.tif"
        )
        ...
        try:
            ...
            print('Failed running detection')
            det_result = dict(
                type="Multiple points", points=[], version={"major": 1, "minor": 0}
            )
            write_json(det_result, f"{self.output_folder}/detected-lymphocytes.json")
...
if __name__ == "__main__":
    ...
    Path(f"{output_folder}/images/breast-cancer-segmentation-for-tils").mkdir(
